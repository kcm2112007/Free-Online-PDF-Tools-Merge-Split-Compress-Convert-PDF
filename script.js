...
// ============================================================
// PDF STANDARD SECURITY HANDLER (RC4, Revision 2, 40-bit key)
// Pure JS, no external crypto library. MD5 and RC4 below were each
// independently verified against official test vectors (RFC 1321 for
// MD5; standard RC4 test vectors) before this file existed. The full
// encrypt/decrypt pipeline was further verified end-to-end against the
// real `qpdf` command-line tool in both directions (our encryption
// decrypted correctly by real qpdf, and a real-qpdf-encrypted file
// decrypted correctly by this code), for both PDF strings and streams,
// including correct rejection of wrong passwords. See README.md.
// ============================================================
function md5(bytes) {
  ...
  var bitLenHigh = Math.floor((msgLen * 8) / 4294967296) >>> 0;
  for (var b = 0; b < 4; b++) withOne.push((bitLenLow >>> (8*b)) & 0xFF);
  for (var b = 0; b < 4; b++) withOne.push((bitLenHigh >>> (8*b)) & 0xFF);
  var a0 = 0x67452301, b0 = 0xefcdab89, c0 = 0x98badcfe, d0 = 0x10325476;
  for (var chunkStart = 0; chunkStart < withOne.length; chunkStart += 64) {
    var chunk = withOne.slice(chunkStart, chunkStart + 64);
    var M = toWords(chunk);
  ...
  [a0, b0, c0, d0].forEach(function (word) {
    for (var i = 0; i < 4; i++) out.push((word >>> (8*i)) & 0xFF);
  });
  return out;
}
...
function rc4(keyBytes, dataBytes) {
  ...
  for (var n = 0; n < dataBytes.length; n++) {
    i2 = (i2 + 1) & 0xFF;
    j2 = (j2 + S[i2]) & 0xFF;
    var tmp2 = S[i2]; S[i2] = S[j2]; S[j2] = tmp2;
    var k = S[(S[i2] + S[j2]) & 0xFF];
    out.push(dataBytes[n] ^ k);
  }
  return out;
}
...
function pdfSecPadPassword(pwBytes) {
  var out = pwBytes.slice(0, 32);
  var need = 32 - out.length;
  for (var i = 0; i < need; i++) out.push(PDF_SEC_PADDING[i]);
  return out;
}
function pdfSecInt32LE(n) {
  var u = n < 0 ? (n >>> 0) : n;
  return [u & 0xFF, (u >>> 8) & 0xFF, (u >>> 16) & 0xFF, (u >>> 24) & 0xFF];
}
function pdfSecComputeO(ownerPwBytes, userPwBytes) {
  var src = (ownerPwBytes && ownerPwBytes.length) ? ownerPwBytes : userPwBytes;
  var padded = pdfSecPadPassword(src.slice());
  var digest = md5(padded);
  var rc4Key = digest.slice(0, 5);
  var paddedUser = pdfSecPadPassword(userPwBytes.slice());
  return rc4(rc4Key, paddedUser);
}
function pdfSecComputeFileKey(userPwBytes, oBytes, permissions, idBytes) {
  var padded = pdfSecPadPassword(userPwBytes.slice());
  var input = padded.concat(oBytes, pdfSecInt32LE(permissions), idBytes);
  return md5(input).slice(0, 5);
}
function pdfSecComputeU(fileKey) {
  return rc4(fileKey, PDF_SEC_PADDING.slice());
}
function pdfSecObjectKey(fileKey, objNum, objGen) {
  var extra = [objNum & 0xFF, (objNum >> 8) & 0xFF, (objNum >> 16) & 0xFF, objGen & 0xFF, (objGen >> 8) & 0xFF];
  var digest = md5(fileKey.concat(extra));
  return digest.slice(0, Math.min(fileKey.length + 5, 16));
}
function pdfSecTransform(fileKey, objNum, objGen, dataBytes) {
  return rc4(pdfSecObjectKey(fileKey, objNum, objGen), dataBytes);
}
function pdfSecBytesToHex(bytes) { return bytes.map(function (b) { return (b < 16 ? "0" : "") + b.toString(16); }).join(""); }
function pdfSecRandomBytes(n) {
...
// ============================================================
// pdf-lib object-graph walking (encrypts/decrypts in place)
// Verified against pdf-lib's actual published source: PDFDict.entries()/
// set(), PDFArray.size()/get()/set(), PDFString.asBytes()/PDFString.of(),
// PDFHexString.asBytes()/PDFHexString.of(), PDFRawStream.dict/.contents.
// ============================================================
function pdfSecTransformValue(PDFLib, value, fileKey, num, gen) {
  if (value instanceof PDFLib.PDFString) {
    var bytes = Array.from(value.asBytes());
    var out = pdfSecTransform(fileKey, num, gen, bytes);
    return PDFLib.PDFString.of(out.map(function (b) { return String.fromCharCode(b); }).join(""));
  }
  if (value instanceof PDFLib.PDFHexString) {
    var bytes2 = Array.from(value.asBytes());
    var out2 = pdfSecTransform(fileKey, num, gen, bytes2);
    return PDFLib.PDFHexString.of(pdfSecBytesToHex(out2));
  }
  if (value instanceof PDFLib.PDFDict || value instanceof PDFLib.PDFArray || value instanceof PDFLib.PDFRawStream) {
    pdfSecTransformObject(PDFLib, value, fileKey, num, gen);
    return value;
  }
  return value; // PDFName, PDFNumber, PDFBool, PDFNull, PDFRef: never encrypted
}
function pdfSecTransformObject(PDFLib, obj, fileKey, num, gen) {
  if (obj instanceof PDFLib.PDFRawStream) {
    obj.contents = new Uint8Array(pdfSecTransform(fileKey, num, gen, Array.from(obj.contents)));
    pdfSecTransformObject(PDFLib, obj.dict, fileKey, num, gen);
    return;
  }
  if (obj instanceof PDFLib.PDFDict) {
    obj.entries().forEach(function (entry) {
      obj.set(entry[0], pdfSecTransformValue(PDFLib, entry[1], fileKey, num, gen));
    });
    return;
  }
  if (obj instanceof PDFLib.PDFArray) {
    for (var i = 0; i < obj.size(); i++) {
      obj.set(i, pdfSecTransformValue(PDFLib, obj.get(i), fileKey, num, gen));
    }
  }
}

async function pdfSecEncrypt(PDFLib, pdfBytes, userPassword, ownerPassword) {
  var pdfDoc = await PDFLib.PDFDocument.load(pdfBytes, { ignoreEncryption: true });
  if (pdfDoc.isEncrypted) throw new Error("This PDF is already password-protected — remove its existing password first.");

  var context = pdfDoc.context;
  var idBytes = pdfSecRandomBytes(16);
  var userPwBytes = Array.prototype.map.call(userPassword, function (c) { return c.charCodeAt(0) & 0xFF; });
  var ownerPwBytes = Array.prototype.map.call(ownerPassword || userPassword, function (c) { return c.charCodeAt(0) & 0xFF; });
  var permissions = -44; // conventional "allow everything" value for a personal-use protection tool

  var oBytes = pdfSecComputeO(ownerPwBytes, userPwBytes);
  var fileKey = pdfSecComputeFileKey(userPwBytes, oBytes, permissions, idBytes);
  var uBytes = pdfSecComputeU(fileKey);

  context.enumerateIndirectObjects().forEach(function (pair) {
    var ref = pair[0], obj = pair[1];
    pdfSecTransformObject(PDFLib, obj, fileKey, ref.objectNumber, ref.generationNumber);
  });

  var encryptDict = context.obj({
    Filter: PDFLib.PDFName.of("Standard"),
    V: 1, R: 2,
    O: PDFLib.PDFHexString.of(pdfSecBytesToHex(oBytes)),
    U: PDFLib.PDFHexString.of(pdfSecBytesToHex(uBytes)),
    P: permissions,
  });
  var encryptRef = context.register(encryptDict);
  context.trailerInfo.Encrypt = encryptRef;
  context.trailerInfo.ID = context.obj([
    PDFLib.PDFHexString.of(pdfSecBytesToHex(idBytes)),
    PDFLib.PDFHexString.of(pdfSecBytesToHex(idBytes)),
  ]);
...
async function pdfSecDecrypt(PDFLib, pdfBytes, password) {
  var pdfDoc = await PDFLib.PDFDocument.load(pdfBytes, { ignoreEncryption: true });
  var context = pdfDoc.context;
  var encryptRef = context.trailerInfo.Encrypt;
  if (!encryptRef) throw new Error("This PDF doesn't appear to be password-protected.");

  var encryptDict = context.lookup(encryptRef, PDFLib.PDFDict);
  var V = encryptDict.get(PDFLib.PDFName.of("V"));
  var R = encryptDict.get(PDFLib.PDFName.of("R"));
  var vNum = V ? V.asNumber() : 0;
  var rNum = R ? R.asNumber() : 0;
  if (!(vNum === 1 && rNum === 2)) {
    throw new Error("This PDF uses a stronger encryption method (128-bit or AES) that this tool doesn't support removing — only basic 40-bit encryption can be removed here.");
  }

  var oObj = encryptDict.get(PDFLib.PDFName.of("O"));
  var uObj = encryptDict.get(PDFLib.PDFName.of("U"));
  var pObj = encryptDict.get(PDFLib.PDFName.of("P"));
  var oBytes = Array.from(oObj.asBytes());
  var uBytes = Array.from(uObj.asBytes());
  var permissions = pObj.asNumber();

  var idArr = context.trailerInfo.ID;
  var idBytes = idArr ? Array.from(context.lookup(idArr.get(0)).asBytes()) : [];

  var pwBytes = Array.prototype.map.call(password, function (c) { return c.charCodeAt(0) & 0xFF; });
  var candidateKey = pdfSecComputeFileKey(pwBytes, oBytes, permissions, idBytes);
  var candidateU = pdfSecComputeU(candidateKey);
  var matches = candidateU.every(function (b, i) { return b === uBytes[i]; });
  if (!matches) throw new Error("Incorrect password.");

  var encryptObjNum = null;
  context.enumerateIndirectObjects().forEach(function (pair) {
    if (pair[0] === encryptRef || (encryptRef.objectNumber !== undefined && pair[0].objectNumber === encryptRef.objectNumber)) {
      encryptObjNum = pair[0].objectNumber;
    }
  });

  context.enumerateIndirectObjects().forEach(function (pair) {
    var ref = pair[0], obj = pair[1];
    if (encryptObjNum !== null && ref.objectNumber === encryptObjNum) return; // never touch the Encrypt dict itself
    pdfSecTransformObject(PDFLib, obj, candidateKey, ref.objectNumber, ref.generationNumber);
  });
...
"use strict";
  ...
  function renderFileList(ulEl, items, onRemove) {
    ...
    items.forEach(function (item) {
      ...
      rm.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true" style="width:14px;height:14px;"><path d="M6 6l12 12M18 6L6 18"/></svg>';
  ...
  function showMsg(el, text, type) {
    ...
    // Errors interrupt immediately; success/info wait for a pause in speech.
  ...
  /**
   * Strip characters that are illegal in filenames on Windows/Android
   * (< > : " / \ | ? * and control characters), while leaving Unicode
   * scripts (Bengali, Hindi, etc.) completely untouched — only the small
   * fixed set of filesystem-reserved ASCII characters is replaced.
   */
...
var C = window.PDFToolsCommon;
  ...
  runBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      var base = C.sanitizeFilename(C.baseName(currentFile.name));
      if (count === 1) {
        outputBlob = blobs[0];
        outputName = base + ".jpg";
      } else {
...
// Protect PDF and Remove PDF Password were previously disabled after two
// failed attempts using a third-party WASM library. They now use a
// from-scratch implementation (see pdf_security.js content inlined below)
// that was independently verified against the real `qpdf` command-line
// tool — round-tripped in both directions, for both PDF strings and
// streams, with correct password rejection — before being wired up here.
// See README.md for exactly what was and wasn't verified.
...
async function mount_protect_pdf(root) {
  ...
  runBtn.addEventListener("click", async function () {
    ...
    try {
      await C.loadScript("https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js");
      C.setProgress(progress, fill, label, 20, "Reading file…");
      var buf = await C.readFileAsArrayBuffer(currentFile);
      C.setProgress(progress, fill, label, 45, "Encrypting…");
      var outBytes = await pdfSecEncrypt(window.PDFLib, buf, pw, pw);
      ...
      result.classList.add("show");
      C.showMsg(msg, "PDF protected successfully. Keep the password somewhere safe — it can't be recovered. Note: this uses standard 40-bit encryption (an older but genuine, universally-openable protection method) — not the strongest modern option, but a real password lock, not a fake one.", "success");
    } catch (err) {
      console.error(err);
      C.showMsg(msg, "Couldn't protect this PDF: " + C.friendlyError(err), "error");
    } finally {
      runBtn.disabled = false;
      C.resetProgress(progress, fill);
    }
...
async function mount_remove_pdf_password(root) {
  ...
  runBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      C.setProgress(progress, fill, label, 20, "Reading file…");
      var buf = await C.readFileAsArrayBuffer(currentFile);
      C.setProgress(progress, fill, label, 45, "Removing password…");
      var outBytes = await pdfSecDecrypt(window.PDFLib, buf, pw);
      outputBlob = new Blob([outBytes], { type: "application/pdf" });
...
function mount_word_to_pdf(root) {
  ...
  runBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      await C.loadScript("https://cdn.jsdelivr.net/npm/mammoth@1.6.0/mammoth.browser.min.js");
      await C.loadScript("https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js");
      ...
      result.classList.add("show");
      C.showMsg(msg, "Converted. Note: the PDF is a visual image of your document, not selectable/searchable text — headings, paragraphs, and formatting will look right, but you can't click-select or search the words. Complex layouts, tables, and exact pagination may also not match the original precisely.", "success");
    } catch (err) {
      console.error(err);
...
var UI_ICONS = {
  lock: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/><circle cx="12" cy="15.5" r="1.3" fill="currentColor" stroke="none"/></svg>',
  camera: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="6" width="18" height="14" rx="3"/><circle cx="12" cy="13" r="4"/><path d="M8 6l1.5-2h5L16 6"/></svg>',
  people: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="9" cy="8" r="3.2"/><path d="M3 20c0-3.3 2.7-6 6-6s6 2.7 6 6"/><circle cx="17.5" cy="9" r="2.4"/><path d="M15.5 20c.2-2.6 2-4.6 4.5-4.9"/></svg>',
  globe: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3a14 14 0 0 1 0 18"/><path d="M12 3a14 14 0 0 0 0 18"/></svg>',
  message: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 5h16v11H8l-4 4V5z"/></svg>',
  instagramLogo: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="2.5" y="2.5" width="19" height="19" rx="5"/><circle cx="12" cy="12" r="4.3"/><circle cx="17.2" cy="6.8" r="1.1" fill="currentColor" stroke="none"/></svg>',
  facebookLogo: '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="10" fill="currentColor"/><path d="M13.6 21.9v-7.6h2.6l.4-3h-3v-1.9c0-.9.2-1.5 1.5-1.5h1.6V5.2c-.3 0-1.2-.1-2.3-.1-2.3 0-3.9 1.4-3.9 4v2.2H8v3h2.5v7.6h3.1z" fill="var(--navy-950)"/></svg>'
};
...
function toolPageHTML(slug) {
  ...
  return [
    ...
    '    <span class="glyph">' + UI_ICONS.lock + '</span>',
...
function homeHTML(filter) {
  ...
  var cards = Object.keys(TOOL_META).map(function (slug) {
    ...
    var unavailable = UNAVAILABLE[slug];
    return '<a class="pdft-toolcard' + (unavailable ? ' pdft-toolcard-disabled' : '') + '" href="#/' + slug + '"' + (hide ? ' hidden' : '') + '>' +
      '<div class="glyph">' + categoryIcon(m.cat) + '</div>' +
      '<span class="cat">' + m.cat + (unavailable ? ' <span class="pdft-badge-unavailable">Unavailable</span>' : '') + '</span>' +
      '<h3>' + m.h1 + '</h3>' +
  ...
  return [
    '<section class="pdft-hero">',
    '  <div class="pdft-container">',
    '    <span class="pdft-eyebrow">20 free tools · No sign-up</span>',
...
function aboutHTML() {
  return [
    ...
    '    <p class="pdft-body">Every tool on this site processes files fully client-side — nothing is uploaded anywhere. The two password tools (Protect PDF, Remove PDF Password) use a from-scratch encryption implementation, written and verified against the real qpdf command-line tool rather than depending on a third-party library, after an earlier version of that approach turned out to be unreliable.</p>',
...
function contactHTML() {
  return [
    ...
    '    <p class="pdft-lede">Found a bug, have a tool request, or just want to say hello? Send a message below, or reach out through any of these.</p>',
    '  </div>',
    '</section>',
    '<section class="pdft-container" style="padding:8px 0 48px;">',
    '  <div class="pdft-grid3">',
    '    <a class="pdft-card pdft-social-card" href="https://www.instagram.com/kcm_0_7" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">',
    '      <div class="glyph" style="color:#E1306C;">' + UI_ICONS.instagramLogo + '</div><h3>Instagram</h3><p>@kcm_0_7</p>',
    '    </a>',
    '    <a class="pdft-card pdft-social-card" href="https://www.facebook.com/share/1HnbKXyqSZ/" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">',
    '      <div class="glyph" style="color:#1877F2;">' + UI_ICONS.facebookLogo + '</div><h3>Facebook</h3><p>Kalicharan Murmu</p>',
    '    </a>',
    '    <a class="pdft-card pdft-social-card" href="https://kcm2112007.github.io/KalicharanMurmu-/" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">',
    '      <div class="glyph">' + UI_ICONS.globe + '</div><h3>Portfolio</h3><p>kcm2112007.github.io</p>',
    '    </a>',
    '  </div>',

    '  <div class="pdft-panel" style="margin-top:24px;">',
    '    <h2 class="pdft-h3" style="margin-top:0;">Send a message</h2>',
    '    <form id="contactForm">',
    '      <div class="pdft-options">',
    '        <div class="pdft-field">',
    ...
    '          <textarea id="cfMessage" name="message" required style="min-height:140px;"></textarea>',
    '        </div>',
    '      </div>',
    '      <div class="pdft-toolbar">',
    '        <button type="submit" class="pdft-btn pdft-btn-primary" id="cfSubmit">Send message</button>',
    '      </div>',
    '      <div class="pdft-msg" id="cfMsg" role="status" aria-live="polite"></div>',
    '    </form>',
    '  </div>',

    '  <div class="pdft-privacy" style="margin-top:20px;">',
    '    <span class="glyph">' + UI_ICONS.message + '</span>',
...
function bindContactForm() {
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      var res = await fetch('https://formspree.io/f/xrpgqkpe', {
        method: 'POST',
        body: formData,
        headers: { 'Accept': 'application/json' }
      });
      if (res.ok) {
        C.showMsg(msgEl, "Message sent — thanks for reaching out. You'll get a reply by email.", 'success');
        form.reset();
      } else {
        var data = await res.json().catch(function () { return null; });
        var detail = data && data.errors && data.errors.length ? data.errors.map(function (er) { return er.message; }).join(' ') : '';
        C.showMsg(msgEl, "Couldn't send your message" + (detail ? ": " + detail : ". Please try again in a moment, or use one of the links above instead.") , 'error');
      }
    } catch (err) {
      console.error(err);
      C.showMsg(msgEl, "Couldn't send your message — check your connection and try again, or use one of the links above instead.", 'error');
    } finally {
...
async function render() {
  ...
  appEl.querySelectorAll(".pdft-dropzone-icon-svg").forEach(function (el) {
    el.innerHTML = categoryIcon(el.getAttribute("data-cat"));
  });
