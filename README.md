...
| 13 | Protect PDF with Password | Encrypt a PDF (own implementation, verified against qpdf) |
| 14 | Remove PDF Password | Decrypt a PDF you know the password for (same engine) |
...
| *(none — see below)* | — | Protect PDF, Remove PDF Password use a from-scratch MD5+RC4 implementation, not a third-party library |
| [mammoth.js](https://github.com/mwilliamson/mammoth.js) | 1.6.0 | Word→PDF (docx → HTML conversion) |
...
- **Protect PDF / Remove PDF Password now use a from-scratch implementation**,
  not a third-party library. Two attempts to use `@neslinesli93/qpdf-wasm`
  both failed in real-world testing (confirmed via user screenshots) — it
  turned out to be built as CommonJS rather than a real ES module, so a raw
  browser `import()` silently received no usable export. Rather than patch
  around it a third time, the dependency was removed and replaced with:
  - A pure-JS **MD5** implementation, verified against all 6 official
    RFC 1321 test vectors.
  - A pure-JS **RC4** implementation, verified against 3 standard test
    vectors plus a symmetry check.
  - The **PDF Standard Security Handler** (RC4, Revision 2, 40-bit key —
    Algorithms 1–4 and 6 from ISO 32000-1), built on the two primitives
    above and independently verified using the real `qpdf` command-line
    tool as ground truth: a file encrypted by this code was successfully
    decrypted by real `qpdf` (confirming the `/O` and `/U` values are
    correct), and a file encrypted by real `qpdf` was successfully
    decrypted by this code — for both PDF strings **and** streams, with
    wrong passwords correctly rejected in both directions.
  - Integration with **pdf-lib's actual object model** (`PDFDict.entries()`/
    `.set()`, `PDFArray.size()`/`.get()`/`.set()`, `PDFString`/`PDFHexString`
    `.asBytes()`/`.of()`, `PDFRawStream.contents`), confirmed against
    pdf-lib's published source rather than assumed.
  - What was **not** possible to verify from this environment: the fully
    assembled pipeline running in an actual browser end-to-end. The
    cryptographic algorithm and the pdf-lib API calls were each verified
    independently and rigorously; their combination should be tested by
    the deployer before relying on it for anything sensitive. This tool
    uses standard 40-bit RC4 encryption (PDF's original, legacy encryption
    method) — a real, universally-openable password lock, but not as
    strong as modern AES-256, so don't use it for highly sensitive
    documents.
